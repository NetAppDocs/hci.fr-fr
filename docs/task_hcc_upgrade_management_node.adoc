...
Si vous effectuez une mise à niveau sur place d'un nœud de gestion existant vers Element 12.8 ou une version ultérieure, avant de procéder à la mise à niveau, vous devez vous assurer que la version matérielle de la machine virtuelle sur le nœud de gestion est compatible avec ESXi 6.7 (version matérielle de la machine virtuelle 14) ou une version ultérieure en fonction de votre environnement.
