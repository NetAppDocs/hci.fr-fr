...
Si vous effectuez une mise à niveau sur place d'un nœud témoin existant vers Element 12.8 ou une version ultérieure, avant de procéder à la mise à niveau, vous devez vous assurer que la version matérielle de la machine virtuelle sur le nœud témoin est compatible avec ESXi 6.7 (version matérielle de la machine virtuelle 14) ou une version ultérieure en fonction de votre environnement. Voirlink:task_hcc_upgrade_management_node.html#upgrade-vm-hardware["Mise à niveau de la version matérielle de la machine virtuelle"] .
